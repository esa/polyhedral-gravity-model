...
namespace polyhedralGravity {
    ...
    GravityModelResult GravityEvaluable::evaluate(const Array3 &computationPoint) const {
         ...
         */
        auto polyhedronIterator = GravityModel::transformPolyhedron(_polyhedron, computationPoint);
    ...
    Array3 GravityEvaluable::buildUnitNormalOfPlane(const Array3 &segmentVector1, const Array3 &segmentVector2) const {
        //Calculate N_i as (G_i1 * G_i2) / |G_i1 * G_i2| with * being the cross product
        return util::normal(segmentVector1, segmentVector2);
    }

    Array3Triplet GravityEvaluable::buildUnitNormalOfSegments(const Array3Triplet &segmentVectors,
                                                              const Array3 &planeUnitNormal) const {
    ...
    double
    GravityEvaluable::computeUnitNormalOfPlaneDirection(const Array3 &planeUnitNormal, const Array3 &vertex0) const {
        using namespace util;
    ...
    HessianPlane GravityEvaluable::computeHessianPlane(const Array3 &p, const Array3 &q, const Array3 &r) const {
        using namespace util;
    ...
    double GravityEvaluable::distanceBetweenOriginAndPlane(const HessianPlane &hessianPlane) const {
    ...
    Array3 GravityEvaluable::projectPointOrthogonallyOntoPlane(const Array3 &planeUnitNormal, double planeDistance,
    ...
    Array3 GravityEvaluable::computeUnitNormalOfSegmentsDirections(const Array3Triplet &vertices,
    ...
    Array3Triplet GravityEvaluable::projectPointOrthogonallyOntoSegments(const Array3 &projectionPointOnPlane,
    ...
    Array3 GravityEvaluable::projectPointOrthogonallyOntoSegment(const Array3 &vertex1, const Array3 &vertex2,
    ...
    Array3 GravityEvaluable::distancesBetweenProjectionPoints(const Array3 &orthogonalProjectionPointOnPlane,
    ...
    std::array<Distance, 3> GravityEvaluable::distancesToSegmentEndpoints(const Array3Triplet &segmentVectorsForPlane,
    ...
    GravityEvaluable::computeTranscendentalExpressions(const std::array<Distance, 3> &distancesForPlane,
    ...
    std::pair<double, Array3> GravityEvaluable::computeSingularityTerms(const Array3Triplet &segmentVectorsForPlane,
    ...
    Array3 GravityEvaluable::computeNormsOfProjectionPointAndVertices(const Array3 &orthogonalProjectionPointOnPlane,
                                                                      const Array3Triplet &face) const {
        using namespace util;
