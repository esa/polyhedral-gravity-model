...
PYBIND11_MODULE(polyhedral_gravity, m) {
    ...
    // TODO: Complete & Add Documentation
    pybind11::class_<GravityEvaluable>(m, "GravityEvaluable")
            .def(pybind11::init<
                         const std::vector<std::array<double, 3>> &, const std::vector<std::array<size_t, 3>> &, double>(),
                 pybind11::arg("vertices"), pybind11::arg("faces"), pybind11::arg("density"))
            .def("evaluate", py::overload_cast<const Array3 &>(&GravityEvaluable::evaluate, py::const_))
            .def("evaluate", py::overload_cast<const std::vector<Array3> &>(&GravityEvaluable::evaluate, py::const_));
    ...
    utility.def("check_mesh",
                [](const std::vector<std::array<double, 3>> &vertices,
                   const std::vector<std::array<size_t, 3>> &faces) {
                    ...
                    return MeshChecking::checkTrianglesNotDegenerated(poly) &&
                           MeshChecking::checkNormalsOutwardPointing(poly);
                }, R"mydelimiter(
